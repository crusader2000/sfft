...
function [set_I,set_h_sigma,set_o_sigma,set_Z,set_omega,set_tau,set_G]=get_the_sets(x,L,n)
	set_I=[]
	set_h_sigma=[]
	set_o_sigma=[]
	set_tau=[]
	...
	set_G=[]
	for i=1:L
		[I,h_sigma,o_sigma,Z,omega,tau,G]=inner_location_loop(x,n)
		append(set_I,I)
		append(set_h_sigma,h_sigma)
		append(set_o_sigma,o_sigma)
		append(set_Z,Z)
		append(set_omega,omega)
		append(set_tau,tau)
		append(set_G,G)
	end
end
...
function set_of_X_r_I_dash=get_x_r_I_dash(I_dash,set_h_sigma,set_o_sigma,set_Z,set_omega,set_tau,set_G)
	set_of_X_r_I_dash=[]
	for i=1:L
		X_estim=find_estimates(set_h_sigma[i],set_o_sigma[i],set_omega[i],set_tau[i],set_G[i],I_dash,set_Z[i])
...
function X=sfft(x,L,n)
		[set_I,set_h_sigma,set_o_sigma,set_Z,set_omega,set_tau,set_G]=get_the_sets(x,L,n)
		[s,union_of_all_sets]=get_s(I,L)
		I_dash=get_I_dash(union_of_all_sets,s,L)
		set_of_X_r_I_dash=get_x_r_I_dash(I_dash,set_h_sigma,set_o_sigma,set_Z,set_omega,set_tau,set_G)
