...
function [x_samp,J] = inner_loop_locate(origx,n, filter_timedo,filter_sizet,filter_freqdo,num,B,a,ai,b)
  ...
  % samples_dash = samples(samples > 20 );
  % disp("samples");
  % disp(samples_dash);
  % disp("length(samples_dash)");
  % disp(length(samples_dash));
  % disp(num);

  % disp("INNER LOOP LOCATE FIND_LARGEST_INDICES");

  % if num > length(samples_dash)
  %   num = length(samples_dash);
  % end

  if num < B
    J= find_largest_indices(num,samples);
    % J= find_largest_indices(num,samples_dash);
  else
    J= find_largest_indices(B,samples);
    % J= find_largest_indices(B,samples_dash);
  end
end
